...
#include "ConsoleReporterPlus.hh"
...
namespace snej_catch {
    ...
    void ConsoleReporter::testCaseStarting(TestCaseInfo const& _testInfo) {
        StreamingReporterBase::testCaseStarting(_testInfo);
        m_stream << std::endl << m_colour->guardColour(Colour::BrightWhite)
    ...
    void ConsoleReporter::printOpenHeader(std::string const& _name) {
        m_stream << std::endl << m_colour->guardColour(Colour::BrightWhite)
            << lineOfChars('-') << " IN TEST " << _name << std::endl;
//        {
//            auto guard = m_colour->guardColour( Colour::Headers ).engage( m_stream );
...
//        }
    ...
    CATCH_REGISTER_REPORTER("consoleplus", ConsoleReporter)
