...
namespace snej::coro::uv {
    ...
    public:
        ...
        [[nodiscard]] Future<void> open(std::string const& path, int flags = ReadOnly, int mode = 0644);
        ...
        [[nodiscard]] Future<size_t> pread(ReadBuf buf, uint64_t offset)  {return preadv(&buf, 1, offset);}
        ...
        [[nodiscard]] Future<void> write(size_t len, const void* src)     {return write(WriteBuf{src, len});}
        [[nodiscard]] Future<void> write(WriteBuf buf)                    {return pwritev(&buf, 1, -1);}

        /// Writes to the file at the given offset.
        [[nodiscard]] Future<void> pwrite(WriteBuf buf, uint64_t offset)  {return pwritev(&buf, 1, offset);}
